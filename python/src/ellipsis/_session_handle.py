...
TERMINAL_STATUSES = frozenset({"closed", "idle", "failed", "cancelled", "stopped"})
...
class SessionHandle:
    ...
    def send(
        ...
        *,
        ...
        boundary; wakes a parked session). `images` ride the message as
        content blocks the model sees on that turn — a paste into Claude
        Code."""
...
def _settled(session: models.Session) -> bool:
    return (
        session.lifecycle.conversation == "closed"
        or session.lifecycle.status in TERMINAL_STATUSES
    )

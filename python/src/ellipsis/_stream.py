"""The session stream WebSocket client (protocol v6) — the Python mirror of
...
SESSION_STREAM_PROTOCOL_VERSION = 6
...
def session_status_word(session: Any) -> str:
    """The canonical status shared by REST and session streams."""
    return str(session.lifecycle.status)
    ...
    *,
    ...
    async def emit(frame: frames.StreamFrame) -> None:
        ...
        if isinstance(frame, frames.RecordsAppendFrame):
            ...
            last_exit_status = (
                frame.session.lifecycle.last_execution_result.completion_reason
                if frame.session.lifecycle.last_execution_result is not None
                else None
            )

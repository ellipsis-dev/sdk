// The session stream WebSocket client (protocol v6, server -> client, one
...
//   delta:          { type, turn_id, kind, text?, output_tokens? }
//   heartbeat:      { type, ts }
...
export const SESSION_STREAM_PROTOCOL_VERSION = 6;
...
// (working/waiting/idle/starting/closed/…) when present, else the raw
...
export function sessionStatusWord(session: Session): string {
  return session.lifecycle.status;
}
...
// (`protocol=6&after_seq=N`) — append it to whatever URL the door uses (plus
...
// The v4 handshake query (§3.1): `?protocol=` is REQUIRED — a server that
...
export async function streamSession(
  ...
  const emit = (frame: StreamFrame) => {
    ...
    if (frame.type === 'records_append') {
      ...
      lastExitStatus =
        session.lifecycle.last_execution_result?.completion_reason ?? null;
    }
    opts.onFrame(frame);
  };

...
// Consecutive tool activity and reasoning share a compact stack. Messages and
// lifecycle notices separate stacks; top-level rows keep their own gutter.
...
export function layOutItems(items: readonly TranscriptItem[]): PlacedItem[] {
  const isActivity = (item: TranscriptItem) =>
    item.kind === 'thinking' || isToolActivity(item);
  return items.map((item, index) => ({
    item,
    nested: false,
    attach: index > 0 && isActivity(item) && isActivity(items[index - 1]),
  }));
}

...
// is a `CodexEvent` (discriminated by `type`), codex_app_server@1 uses native
// `method` tags, and a lifecycle payload renders as a notice line.
// This module is pure (no ANSI, no DOM) — colours, glyphs, and
...
export interface TranscriptItem {
  ...
  kind: ItemKind;
  text: string;
  // The source message's timestamp, when the record has committed.
  ...
  loading?: boolean;
  // A thinking row that stands for a whole turn's thoughts ("Thinking" while
  // it runs, "Thought for 41s" after) rather than one thought. Its `text` IS
  // the label; a renderer shows no thought body beside it.
  summary?: boolean;
}
  ...
  if (event.kind === 'user') {
    for (const block of blocks) {
      if (block.type === 'tool_result') {
        const body = toolResultText(block.content ?? null);
        push({
          kind: 'tool_result',
          gutter: '⎿',
          text: body || '(no output)',
...
export function recordToItems(
  ...
  switch (record.kind) {
    case 'claude_sdk':
    case 'claude_code': {
      const payload = claudePayload(record);
      return payload ? eventToItems(payload, keyBase, options) : [];
    }
    case 'codex':
    ...
    case 'codex_app_server': {
      ...
      if (content.kind === 'tool') {
        const summary = oneLine(content.summary, 100);
        return [
          {
            key: `${keyBase}:0`,
            kind: 'tool',
            ...
            detail: summary ? `(${summary})` : undefined,
            spaceBefore: true,
            tool: { name: content.name, input: content.input ?? undefined },
          },
          {
            key: `${keyBase}:1`,
            kind: 'tool_result',
            gutter: '⎿',
            text: content.result || '(completed)',

...
export function isSettled(session: S['Session']): boolean {
  return (
    ...
    TERMINAL_STATUSES.has(session.lifecycle.status)
  );
}
...
export class SessionHandle {
  ...
  // wakes a parked session). `images` ride the message as content blocks the
  // model sees on that turn — a paste into Claude Code.
  async send(
    message: string,
